...
class Labelprint
{
	...
	*/
	function generate_barcode($id_prod)
	{
		...
		$encoding = "EAN-13";
		while($loop){
			$ean = rand(200000,299999).rand(0,999999);
...
class pdfLabel
{
	...
	function createPdf()
	{
		...
		if ($conf->global->MAIN_MODULE_LABELPRINT_LABELS_0){
			...
			$PosY=12+(floor($lab_start/5)*21);
			$PosX=7+($lab_start % 5)*42;
		}
			...
			while ($i < $num)
			{
				...
				while($n<$qty)
				{
					if($conf->global->MAIN_MODULE_LABELPRINT_LABELS_0 || $conf->global->MAIN_MODULE_LABELPRINT_LABELS_1){
						...
						$pdf->SetFillColor(230,230,230);
						if($conf->global->LAB_COMP){
							$pdf->MultiCell(65,5,dol_trunc($mysoc->nom,14),0,'L');
						}
						elseif($conf->global->LAB_PROD_LABEL){
							$pdf->MultiCell(65,5,dol_trunc($product->libelle,14),0,'L');
						}
						else{
							$pdf->MultiCell(65,5,dol_trunc($conf->global->LAB_FREE_TEXT,14),0,'L');
						}
						//Position Y
						$PosYLabel=$PosY+$Line;
						//$PosYLabel=$PosYLabel+$Line+2;
						...
						if ($conf->barcode->enabled)
						{
							...
							$pdf->write1DBarcode($product->barcode, $product->barcode_type_code, '', '', 31, 16, 0.4, $style, 'N');
