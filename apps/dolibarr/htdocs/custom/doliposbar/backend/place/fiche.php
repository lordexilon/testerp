...
/**
 *	    \file       htdocs/doliposbar/backend/place/fiche.php
 *      \ingroup    pos
...
dol_include_once('/doliposbar/backend/lib/place.lib.php');
...
if (GETPOST('action','alpha') == 'add')
{
	...
	if (! $error)
    {
        ...
        if ($id == 0)
        {
           $url=dol_buildpath("/doliposbar/backend/place/place.php",1);
           Header("Location: ".$url); 	
        }
        else 
        {
...
if (GETPOST('action','alpha') == 'confirm_delete' && GETPOST('confirm','alpha') == "yes" && $user->rights->pos->backend)
{
    ...
    header("Location: ".dol_buildpath("/doliposbar/backend/place/place.php",1));
    exit;
}
