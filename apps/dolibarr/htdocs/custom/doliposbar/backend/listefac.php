...
/**
 *	\file       htdocs/doliposbar/backend/listefac.php
 *	\ingroup    factures
...
{
	print '<a href="'.dol_buildpath('/doliposbar/frontend/index.php',1).'"><img src='.dol_buildpath('/doliposbar/frontend/img/bgback.png',1).' WIDTH="100%" HEIGHT="100%" ></a>';
}	
else {
	if ($page == -1) $page = 0 ;
	 ...
	 $sql = 'SELECT';
        ...
        if ($resql)
        {
			...
			if ($closeid)
			{
				$url = '../pro/tpl/closecash.tpl.php?id='.$closeid.'&terminal='.$terminalid;
				print '<a class="butAction" href='.$url.' target="_blank">'.$langs->trans('PrintCopy').'</a>';
				
				print '<a class="butAction" href="'.dol_buildpath('/doliposbar/backend/listefac.php',1).'?closeid='.$closeid.'&terminalid='.$terminalid.'&viewstatut=2&action=mail">'.$langs->trans('MailCopy').'</a>';
