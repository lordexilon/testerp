...
class POS extends CommonObject
{
	...
	{	
		...
		if($idCat>0)
		{
			...
			$dir = $conf->categorie->multidir_output[1].'/'.$pdir;
			foreach ($objCat->liste_photos($dir,1) as $key => $obj)
			{
				...
				$realpath = DOL_URL_ROOT.'/viewimage.php?modulepart=category&file='.urlencode($pdir.$filename) ;
	...
	public static function SearchProduct($idSearch,$stock=false, $warehouse,$mode=0, $ticketstate=0)
	{
		...
		if($mode>=0){
			...
			if(!$stock && $conf->global->POS_SERVICES)
			{
				...
				$sql.=") OR (p.tosell = 1 ) AND p.fk_product_type = 1";
				$sql.= " AND (p.ref LIKE '%".$idSearch."%' OR p.label LIKE '%".$idSearch."%' ";
				if ($conf->barcode->enabled) $sql.= " OR p.barcode='".$idSearch."')";
				else $sql.= ")";
				if(!$conf->global->POS_STOCK){
		...
		$sql= "SELECT distinct p.rowid, p.ref, p.label , w.reel as stock, w.fk_entrepot as warehouseId, e.label as warehouse ";
				...
				$sql.= " WHERE w.fk_entrepot=".$warehouse;
				if(!$conf->global->POS_SERVICES)
				{
					$sql.= " AND p.fk_product_type = 0";
				}
			...
			if($mode == -5){//best sell
				...
				$sql.=" WHERE ep.rowid = ".$warehouse." and pr.tosell = 1 AND f.rowid = fd.fk_facture AND pr.rowid = fd.fk_product";
			...
			if ($mode == -6){//worst sell
				...
				$sql.= " WHERE p.tosell = 1 AND f.rowid = fd.fk_facture AND p.rowid = fd.fk_product";
				$sql.= " AND (pr.ref LIKE '%".$idSearch."%' OR pr.label LIKE '%".$idSearch."%' ";
				if ($conf->barcode->enabled) $sql.= " OR pr.barcode='".$idSearch."')";
				else $sql.= ")";
				if(!$conf->global->POS_SERVICES)
		...
		if ($resql)
		{
			...
			if($mode == -6 && $num < 10){
				...
				$sql.= " WHERE p.tosell = 1 AND fac.rowid = facd.fk_facture ";
	...
	{	
		...
		if($idProd>0)
		{
			...
			$dir = $conf->product->multidir_output[1].'/'.$pdir;
			...
			foreach ($objProd->liste_photos($dir,1) as $key => $obj)
			{
				...
				$realpath = DOL_URL_ROOT.'/viewimage.php?modulepart=product&file='.urlencode($pdir.$filename) ;
				...
				if (!$obj->fk_societe)
				{
					...
					if($userstatic->rights->pos->frontend)
					{
						...
						if ($file && file_exists($dir."/".$file)){
							$internalusers[$i]['photo'] = DOL_URL_ROOT.'/viewimage.php?modulepart=userphoto&file='.urlencode($file);
						}
						else{
	...
	public static function select_Terminals()
    {
		...
		$sql.= " WHERE is_used = 0 OR (is_used=1 AND is_closed=1)";
	...
	public static function countHistoric()
	{
		...
		$sql.= " WHERE f.date_ticket BETWEEN '".$todayini."' AND '".$todayfin."' ) as today, ";
		...
		$sql.= " WHERE f.date_ticket BETWEEN '".$yestini."' AND '".$yestfin."' ) as yesterday, ";
		...
		$sql.= " WHERE f.date_ticket BETWEEN '".$weekini."' AND '".$weekfin."' ) as thisweek, ";
		...
		$sql.= " WHERE f.date_ticket BETWEEN '".$lweekini."' AND '".$lweekfin."' ) as lastweek, ";
		...
		$sql.= " WHERE f.date_ticket BETWEEN '".$ini2week."' AND '".$fin2week."' ) as twoweek, ";
		...
		$sql.= " WHERE f.date_ticket BETWEEN '".$ini3week."' AND '".$fin3week."' ) as threeweek, ";
		...
		$sql.= " WHERE f.date_ticket BETWEEN '".$monthini."' AND '".$monthfin."' ) as thismonth, ";
		...
		$sql.= " WHERE f.date_ticket BETWEEN '".$monthagoini."' AND '".$monthagofin."' ) as monthago, ";
		...
		$sql.= " WHERE f.date_ticket BETWEEN '".$lmonthini."' AND '".$lmonthfin."' ) as lastmonth";
	...
	public static function getConfig()
	{
		...
		if ($userstatic->photo) $file=get_exdir($userstatic->id,2).$userstatic->photo;
		if ($file && file_exists($dir."/".$file)){
		$ret['data']['user']['photo'] = DOL_URL_ROOT.'/viewimage.php?modulepart=userphoto&file='.urlencode($file);
		}
		else{
