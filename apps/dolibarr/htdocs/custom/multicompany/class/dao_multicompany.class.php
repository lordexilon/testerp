...
class DaoMulticompany extends CommonObject
{
	...
	function getEntities($login=0)
	{
		...
		{
			$sql = "SELECT DISTINCT(rowid)"; // Distinct parce que si user dans plusieurs groupes d'une entité, la liste d'entités de la petite terre affiche plusieurs fois la même entité
			$sql.= " FROM ".MAIN_DB_PREFIX."entity";
			$sql.= " ORDER by rowid";
		}
		else
