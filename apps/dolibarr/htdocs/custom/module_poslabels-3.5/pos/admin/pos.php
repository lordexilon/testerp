...
if (GETPOST('action','string') == 'updateMask')
{
    ...
    if ($maskconstticket) dolibarr_set_const($db,$maskconstticket,$maskticket,'chaine',0,'');
    if ($maskconstticketcredit) dolibarr_set_const($db,$maskconstticketcredit,$maskcredit,'chaine',0,'');
    if ($maskconstfacsim) dolibarr_set_const($db,$maskconstfacsim,$maskfacsim,'chaine',0,'');
    if ($maskconstfacsimcredit) dolibarr_set_const($db,$maskconstfacsimcredit,$maskfacsimcredit,'chaine',0,'');
    if ($maskconstclosecash) dolibarr_set_const($db,$maskconstclosecash,$maskclosecash,'chaine',0,'');
    if ($maskconstclosecasharq) dolibarr_set_const($db,$maskconstclosecasharq,$maskclosecasharq,'chaine',0,'');
}

if (GETPOST("action") == 'set')
{
	$db->begin();
	$res = dolibarr_set_const($db,"POS_SERVICES", GETPOST("POS_SERVICES"),'chaine',0,'');

	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_PLACES", GETPOST("POS_PLACES"),'chaine',0,'');
	
	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_USE_TICKETS", GETPOST("POS_USE_TICKETS"),'chaine',0,'');
			
	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_STOCK", GETPOST("POS_STOCK"),'chaine',0,'');
	
	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_MAX_TTC", GETPOST("POS_MAX_TTC"),'chaine',0,'');

	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_PRINT", GETPOST("POS_PRINT"),'chaine',0,'');
	
	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_MAIL", GETPOST("POS_MAIL"),'chaine',0,'');
	
	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_FACTURE", GETPOST("POS_FACTURE"),'chaine',0,'');
	
	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"REWARDS_POS", GETPOST("REWARDS_POS"),'chaine',0,'');
	
	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_CHAT", GETPOST("POS_CHAT"),'chaine',0,'');
	
	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_INV", GETPOST("POS_INV"),'chaine',0,'');

	if (! $res > 0) $error++;
	$res = dolibarr_set_const($db,"POS_PREDEF_MSG", GETPOST("POS_PREDEF_MSG"),'chaine',0,'');
...
{
    dolibarr_set_const($db, "TICKET_ADDON",$_GET["value"],'chaine',0,'');
}
if ($_GET["action"] == 'setmodfacsim')
{
	dolibarr_set_const($db, "FACSIM_ADDON",$_GET["value"],'chaine',0,'');
}
if ($_GET["action"] == 'setmodclosecash')
{
	dolibarr_set_const($db, "CLOSECASH_ADDON",$_GET["value"],'chaine',0,'');
}
