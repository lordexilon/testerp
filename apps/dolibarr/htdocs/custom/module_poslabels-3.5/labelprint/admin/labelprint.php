...
if (GETPOST("save"))
{
	...
	$res+=dolibarr_set_const($db,'LAB_COMP',trim(GETPOST("labComp")),'chaine',0,'');
	$res+=dolibarr_set_const($db,'LAB_PROD_LABEL',trim(GETPOST("labProdLabel")),'chaine',0,'');
	$res+=dolibarr_set_const($db,'LAB_FREE_TEXT',trim(GETPOST("labFreeText")),'chaine',0,'');
	$res+=dolibarr_set_const($db,'LAB_WEIGHT',trim(GETPOST("labWeight")),'chaine',0,'');
	$res+=dolibarr_set_const($db,'LAB_LENGTH',trim(GETPOST("labLength")),'chaine',0,'');
	$res+=dolibarr_set_const($db,'LAB_AREA',trim(GETPOST("labArea")),'chaine',0,'');
	$res+=dolibarr_set_const($db,'LAB_VOLUME',trim(GETPOST("labVolume")),'chaine',0,'');
	$res+=dolibarr_set_const($db,'LAB_COUNTRY',trim(GETPOST("labCountry")),'chaine',0,'');
	$res+=dolibarr_set_const($db,'LAB_START',0,'chaine',0,'');
...
if ($action == 'setlabel')
{
	...
	if($status==1)
	{
		...
		if (dolibarr_set_const($db, $labelid,$menutab,'chaine',0,'') > 0)
		{
			Header("Location: ".$_SERVER["PHP_SELF"]);
			exit;
		}
...
if ($action == 'settypelabel')
{
	...
	if($status==1)
	{
		...
		if (dolibarr_set_const($db, $labelid,1,'chaine',0,'') > 0)
		{
			Header("Location: ".$_SERVER["PHP_SELF"]);
			exit;
		}
...
// read const
$labcomp = dolibarr_get_const($db,"LAB_COMP");
$labprodlabel = dolibarr_get_const($db,"LAB_PROD_LABEL");
$labweight = dolibarr_get_const($db,"LAB_WEIGHT");
$lablength = dolibarr_get_const($db,"LAB_LENGTH");
$labarea = dolibarr_get_const($db,"LAB_AREA");
$labvolume = dolibarr_get_const($db,"LAB_VOLUME");
$labcountry = dolibarr_get_const($db,"LAB_COUNTRY");
...
dol_fiche_head($head, 'configuration', $langs->trans("Labels"), 0, 'barcode');
