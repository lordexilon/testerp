	...
	calculeTotal : function(){
		...
		for (var i in this.lines)
		{
			...
			if(parseFloat(line["remise_percent_global"]) != parseFloat(discount))
				...
				{
					sum = parseFloat(sum) + Math.round(parseFloat(this.lines[i].total)*100)/100;
	...
	setLineByIdProducts:function(idProduct){
		...
		if(result.length>0){
			...
			if(globalTicket!=undefined){
				...
				this.price = globalTicket.data.lines[num].price;
				this.price_ttc = globalTicket.data.lines[num].price * (1 + (globalTicket.data.lines[num].tva_tx/100));
				this.total = globalTicket.data.lines[num].total_ttc;
...
var TPV = jQuery.Class({
	...
	searchProduct: function(){
    	...
    	$.each(result, function(id, item) {
    	    $('#id_selectProduct').append(
    	        $('<option></option>').val(item['id']).html(item['label']+' --> '+displayPrice(item['price_ttc']))
    	    );
    	});
    	if(_TPV.barcode==1)
    	{
