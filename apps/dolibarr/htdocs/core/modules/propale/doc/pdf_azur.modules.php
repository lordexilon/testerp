...
class pdf_azur extends ModelePDFPropales
{
	...
	function __construct($db)
	{
		...
		{
			$this->posxtva=99; // IVA PORCENTAJE POR PRODUCTO
			$this->posxup=114;
			$this->posxqty=133; // CANTIDAD
			$this->posxunit=150; // 
		}
		else
		{
		...
		$this->posxdiscount=162; // DESCUENTO POR PRODUCTO
		$this->postotalht=174;  // TOTAL SIN IVA POR PRODUCTO
		if (! empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT)) $this->posxtva=$this->posxup;
		$this->posxpicture=$this->posxtva - (empty($conf->global->MAIN_DOCUMENTS_WITH_PICTURE_WIDTH)?20:$conf->global->MAIN_DOCUMENTS_WITH_PICTURE_WIDTH);	// width of images
		if ($this->page_largeur < 210) // To work with US executive format
		{
			$this->posxpicture-=20;//imagen
			$this->posxtva-=20; //impuesto
			$this->posxup-=20; 
			$this->posxqty-=20; //cantidad
			$this->posxdiscount-=20; // descuento porcentaje
			$this->postotalht-=20; // total sin iva
		}

		$this->tva=array(); //IMPUESTO
		$this->localtax1=array(); //impuesto
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($object->ref_client)
		{
			...
			$pdf->MultiCell(300, 3, $outputlangs->transnoentities("RefCustomer")." : " . $outputlangs->convToOutputCharset($object->ref_client), '', 'R');
		}
