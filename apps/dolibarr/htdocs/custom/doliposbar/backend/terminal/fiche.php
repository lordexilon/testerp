...
/**
 *	    \file       htdocs/doliposbar/backend/terminal/fiche.php
 *      \ingroup    pos
...
dol_include_once('/doliposbar/backend/lib/cash.lib.php');
...
if ($_POST["action"] == 'add')
{
    ...
    if (! $error)
    {
        ...
        if ($id == 0)
        {
           $url=dol_buildpath("/doliposbar/backend/terminal/cash.php",1);
           Header("Location: ".$url); 	
        }
        else 
        {
...
if (GETPOST('action','string') == 'confirm_delete' && GETPOST('confirm','string') == "yes" && $user->rights->pos->backend)
{
    ...
    //header("Location: ".DOL_URL_ROOT."/doliposbar/backend/terminal/cash.php");
    header("Location: ".dol_buildpath("/doliposbar/backend/terminal/cash.php",1));
    exit;
}
...
if ($action == 'create')
{
	...
	print '<td colspan="3">';
	print $form->select_types_paiements(isset($_POST["ModeBank_extra"])?$_POST["ModeBank_extra"]:1,'modebank_extra','', 0);
	print '</td></tr>';
...
else
{
    ...
    if (GETPOST("id") && $action == 'edit' && $user->rights->pos->backend)
    {
		...
		if ($conf->banque->enabled)
		{
			...
			print '<tr><td valign="top" >'.$langs->trans("PaymentBankExtra").'</td>';
			print '<td colspan="3">';
			print $form->select_comptes($cash->fk_paybank_extra,'bank_extra',0, $filtrebank);
			print '</td></tr>';
