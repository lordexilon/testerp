<?php
/* Copyright (C) 2002-2006 Rodolphe Quiedeville  <rodolphe@quiedeville.org>On renomme repertoire facture ($this->ref = ancienne ref, $num = nouvelle ref)
		                // afin de ne pas perdre les fichiers attachesv
 * Copyright (C) 2004      Eric Seigne           <eric.seigne@ryxeo.com>
 * Copyright (C) 2004-2013 Laurent Destilleur   <eldy@users.sourceforge.net>
...
require_once DOL_DOCUMENT_ROOT . '/facturacion/facturearg.class.php';
require_once DOL_DOCUMENT_ROOT . '/facturacion/generarFacturas.class.php';
require_once DOL_DOCUMENT_ROOT . '/compta/paiement/class/paiement.class.php';
require_once DOL_DOCUMENT_ROOT . '/facturacion/modules_facture.php';
...
else if ($action == 'confirm_deleteline' && $confirm == 'yes' && $user->rights->facture->creer)
{
	...
	if ($result > 0)
	{
		...
		if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
		{
			...
			$result=facture_pdf_create_arg($db, $object, $object->modelpdf, $outputlangs, $hidedetails, $hidedesc, $hideref, $hookmanager);
		}
		if ($result >= 0)
		{
			header('Location: '.$_SERVER["PHP_SELF"].'?facid='.$id);
...
else if($action == 'factura_arg')
{
	$object->fetch($id);
	
	
	//echo "id: ".$id."<br>";
	$facturas=new factura_preimpresa($id);
	//echo "Factura: ".$facturas->idDoc;
	//facture_pdf_create_arg($db, $object, $object->modelpdf, $outputlangs, $hidedetails, $hidedesc, $hideref, $hookmanager);
...
else if ($action == 'confirm_valid' && $confirm == 'yes' && $user->rights->facture->valider)
{
    ...
    if (! $error)
    {
    	...
    	//$facturas=new factura_preimpresa($id, $db);
    	//$facturas->generarFacturas();
    	//echo $facturas->sSQLTemp;
    	//exit;
        ...
        if ($result >= 0)
        {
            ...
            if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE))
            {
            	// Aqui se debe crear la cantidad de facturas necesarias
            	
            	$object->fetch($id);    // Reload to get new records
            	$cantItems=count($object->lines);
            	$itemsFact=$conf->global->CANT_LINEAS_FC;
            	$cantFact=ceil($cantItems/$itemsFact);       
            	//echo "Cantidad de Items: ".$cantItems."<br>";
            	//echo "Items por Factura: ".$itemsFact."<br>";
            	//echo "Cantidad de Facturas ".$cantFact."<br>";
            	$nuevasFacturas=new factura_preimpresa();
            	
            	$nuevasFacturas->setSoc($socid);
            	
            	$aFacturas=$nuevasFacturas->genNewFactura($id);
            	$strfacturas="";
            	foreach($aFacturas as $factura=>$valor)
            	{
            		//echo "factura a buscar: ".$valor."<br>";
            		$strListaFacturas.=$valor.",";
            		$object->fetch($valor);
            		//echo "factura a imprimir: ".$object->id."<br>";
            	...
            	$strListaFacturas=substr(trim($strListaFacturas), 0, strlen(trim($strListaFacturas))-1);
            	
            	 $idPedido=$nuevasFacturas->getPedido($id);
            	//header("Location:".DOL_URL_ROOT."/facturacion/list.php?rango=$strListaFacturas");
            	header("Location:".DOL_URL_ROOT."/commande/fiche.php?id=".$idPedido); 
...
else if ($action == 'add' && $user->rights->facture->creer)
{
    ...
    if ($_POST['type'] == 1)
    {
        ...
        if (! $error)
        {
            ...
            $id=$object->createFromOrden($user);
...
else if ($action == 'up' && $user->rights->facture->creer)
{
    ...
    if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE)) facture_pdf_create_arg($db, $object, $object->modelpdf, $outputlangs, $hidedetails, $hidedesc, $hideref, $hookmanager);
...
if ($action == 'create')
{
	...
	// Aqui comienza facturacion desde pedido AL
    ...
    $liste=ModelePDFFactArg::liste_modeles($db);
    ...
    if ($result > 0)
    {
        ...
        else
        {
            ...
            if (! $file || ! is_readable($file))
            {
                ...
                $result=facture_pdf_create_arg($db, $object, GETPOST('model')?GETPOST('model'):$object->modelpdf, $outputlangs, $hidedetails, $hidedesc, $hideref, $hookmanager);
                if ($result <= 0)
                {
                    dol_print_error($db,$result);
                    exit;
