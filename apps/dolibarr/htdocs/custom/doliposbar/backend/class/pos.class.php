...
dol_include_once('/doliposbar/backend/class/payment.class.php');
require_once(DOL_DOCUMENT_ROOT."/categories/class/categorie.class.php");
dol_include_once('/doliposbar/backend/class/cash.class.php');
dol_include_once('/doliposbar/backend/lib/errors.lib.php');
dol_include_once('/doliposbar/backend/class/place.class.php');
require_once(DOL_DOCUMENT_ROOT."/core/lib/company.lib.php");
require_once(DOL_DOCUMENT_ROOT."/core/lib/date.lib.php");
require_once (DOL_DOCUMENT_ROOT.'/compta/facture/class/facture.class.php');
dol_include_once('/doliposbar/backend/class/facturesim.class.php');
...
class POS extends CommonObject
{
	...
	{	
		...
		if($idCat>0)
		{
			...
			foreach ($objCat->liste_photos($dir,1) as $key => $obj)
			{
				...
				$realpath = DOL_URL_ROOT.'/viewimage.php?cache=yes&modulepart=category&entity='.$objCat->entity.'&file='.urlencode($pdir.$filename) ;
				
			}
			if(!$realpath)
			{
				$realpath = DOL_URL_ROOT.'/viewimage.php?cache=yes&modulepart=product&file='.urlencode('noimage.jpg');
			}
			return $realpath;
		}
	...
	{	
		...
		if($idProd>0)
		{
			...
			foreach ($objProd->liste_photos($dir,1) as $key => $obj)
			{
				...
				$realpath = DOL_URL_ROOT.'/viewimage.php?cache=yes&modulepart=product&entity='.$objProd->entity.'&file='.urlencode($pdir.$filename) ;
	...
	Private function CreateFacture($aryTicket)
	{
		...
		$object->socid= $socid;
		$object->statut = $data['state'];
		$object->fk_cash = $conf->global->POS_DEFAULT_CASH;
		$object->remise_percent = $data['discount_percent'];
	...
	function checkPassword($login,$password){
		dol_include_once('/doliposbar/frontend/class/auth.class.php');
