...
/**
 *	\file       htdocs/doliposbar/backed/ticket.php
 *	\ingroup    pos
...
dol_include_once('/doliposbar/backend/class/pos.class.php');
require_once(DOL_DOCUMENT_ROOT."/core/lib/functions2.lib.php");
require_once(DOL_DOCUMENT_ROOT."/core/lib/date.lib.php");
dol_include_once('/doliposbar/backend/lib/ticket.lib.php');
dol_include_once('/doliposbar/backend/class/place.class.php');
...
if (GETPOST('action','alpha') == 'confirm_deleteticket' )
{
	...
	header('Location: '.dol_buildpath('/doliposbar/backend/liste.php',1).'?id='.$id);
}
...
if ($result > 0)
{
	...
	*/
	...
	if ($object->statut>0 && $object->statut<3)
	{
		...
		{
			print '<a class="butAction" href="'.dol_buildpath('/doliposbar/backend/ticket.php?action=facture&id='.$object->id,1).'">'.$langs->trans('CreateBill').'</a>';
		}
		else{
		...
		else 
		{
			...
			print '<a class="butAction" href="'.dol_buildpath('/doliposbar/backend/ticket.php?action=abandon&id='.$object->id,1).'">'.$langs->trans('ClassifyCanceled').'</a>';
		}
		$url = '../frontend/tpl/ticket.tpl.php?id='.$object->id;
		print '<a class="butAction" href='.$url.' target="_blank">'.$langs->trans('PrintCopy').'</a>';
		
		print '<a class="butAction" href="'.dol_buildpath('/doliposbar/backend/ticket.php',1).'?id='.$object->id.'&action=mail">'.$langs->trans('MailCopy').'</a>';
	
	}
	if ($object->statut == 3)
	{
		print '<a class="butAction" href="'.dol_buildpath('/doliposbar/backend/ticket.php?action=reopen&id='.$object->id,1).'">'.$langs->trans('ReOpen').'</a>';
	}
	if ($object->is_erasable())
	{
		print '<a class="butAction" href="'.dol_buildpath('/doliposbar/backend/ticket.php?action=delete&id='.$object->id,1).'">'.$langs->trans('Delete').'</a>';
	}
	else
	{
		print '<a class="butActionRefused" href="#" title="'.$langs->trans("DisabledBecauseNotErasable").'">'.$langs->trans('Delete').'</a>';
	}
	print '<a class="butAction" href="'.dol_buildpath('/doliposbar/backend/liste.php',1).'">'.$langs->trans('Back').'</a>';
	print '</div>';
